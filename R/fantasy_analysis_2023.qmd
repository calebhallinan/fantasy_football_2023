---
title: "Example Analysis (An Analysis on my 2023 Fantasy Football League (So Far))"
...
However, not everything worked the first try and I had to make some changes to his code to load in all the data without some errors. Feel free to check [my github repo](https://github.com/calebhallinan/fantasy_football_2023) for what I did there. So, after some tweeks I was successfully able to read in and save everything I needed to a .csv file in my local directory which I then pushed to my github. Here is a data dictionary for the variables I was using with two different datasets:
...
data|>
  ...
  # plot color between lines
  # geom_ribbon(aes(ymin=projected_weekly_score,ymax=weekly_score,fill=TeamName), alpha=0.5,linetype = 0) +
  # geom_ribbon(aes(ymin=pmin(projected_weekly_score,weekly_score), ymax=weekly_score), 
  #             fill="green", alpha=0.5,linetype = 0) +
  #   geom_ribbon(aes(ymin=weekly_score, ymax=pmax(projected_weekly_score, weekly_score)), 
  #             fill="red", alpha=0.5,linetype = 0) +

  # geom_ribbon(data = . %>% filter(weekly_score >= projected_weekly_score), aes(ymin=projected_weekly_score,ymax=weekly_score),fill="green", alpha=0.5,linetype = 0) +

    # geom_ribbon(data = . %>% filter(weekly_score <= projected_weekly_score), aes(ymin=weekly_score,ymax=projected_weekly_score,fill="red"), alpha=0.5,linetype = 0) +
  ...
  guides(color = "none", fill = "none", linetype = guide_legend(title = "Scores")) +
  # use classic theme
  theme_classic() +
...
ggplot(pts_against, aes(x = Week, color = TeamName)) +
  ...
  labs(
    ...
    subtitle = str_wrap("There is a diverse range of teams, including consistent ones, teams with a lot of boom 
                        potential, and teams with a lot of bust potential. Notable outliers are Team GM, who f
                        requently scores high, and Team TP, who frequently scores low. Generally, teams tend to 
...
matchups_diff |> 
  ...
  labs(
    ...
    subtitle = str_wrap("It appears that there are four distinct tiers of total loss differential observed thus far in the
                        ...
                        in each loss they have.", width = 90)) +
  # get rid of legend
  guides(fill = "none")
...
```{r}
# violinplot of scores for all players and teams

data |> 
  # filter(TeamName == "Team Caleb") |> 
  ...
  labs(
    ...
    fill = "Team Name",
    caption = "Data Source: ESPN Fantasy Website",
    subtitle = str_wrap("The violin plots display the distribution of all player scores for each team. The white line
                        within each plot represents the median score for that team. Notably, Team DH and Team RW have
                        had the best weeks by a player so far this season, while Team WR has had the lowest scores. 
                        Most teams exhibit a similar median score, however, Team GM stands out with a noticeably 
                        higher median compared to other teams.", width = 87)) +
  # get rid of legend
...
data |> 
  filter(PlayerRosterSlot == "Bench") |> 
  group_by(TeamName) |> 
  summarise(bench_points = sum(PlayerScoreActual)) |> 
  ggplot(aes(x = TeamName, y = bench_points, fill = TeamName)) +
  geom_col() +
  scale_fill_manual(values = paletteer_d("ggprism::colors", 12))+
  guides(fill = "none") +
  labs(
